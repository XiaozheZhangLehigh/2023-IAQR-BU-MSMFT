...
mpd_stats = pd.read_csv("mpd_stats.csv", skiprows=2, header = 1)
...
mpd_stats_weekly.to_csv('mpd_stats_cleaned.csv', index=True)
...
##（2）然后取出来sp6m和sp12m，因为methodology文件说已经全都整理成6个月后的预测了 文件名"mpd_stats_cleaned"
mpd_stats_weekly.info
mpd_stats_weekly.isnull().sum()
...
# Define the ticker symbol for corn futures; this is an example and may need to be adjusted
ticker_symbol = 'ZW=F'  # Make sure to use the correct ticker symbol for your specific contract

# Define the period for which you want the historical data
...
# Set the series ID for CPI (e.g., "CPIAUCSL" for CPI for All Urban Consumers: All Items in U.S. City Average)
series_id = 'CPIAUCSL'
